...
class Rule:
    ...
    def __init__(
        ...
        self.name = name
        self.conditions = (
            conditions  # [{'metric': 'cpu_usage', 'op': '>', 'value': 90, 'duration': 3}]
        )
        ...
        for cond in self.conditions:
            ...
            evidence_steps.append(
                EvidenceStep(
                    ...
                    metric=metric_path,
                    value=value,
                    baseline=baseline.get(metric_path, {}).get("mean")
                    if metric_path in baseline
                    else None,
                )
            )
    ...
    def _extract_affected_objects(self, metrics: dict) -> list:
        ...
        if self.anomaly_type == AnomalyType.CPU_ANOMALY:
            ...
            if per_process:
                # 取CPU最高的进程
                sorted_procs = sorted(
                    per_process.values(), key=lambda x: x.get("cpu_percent", 0), reverse=True
                )
            ...
            if futex:
                sorted_procs = sorted(
                    futex.values(), key=lambda x: x.get("avg_wait_ms", 0), reverse=True
                )
                for proc in sorted_procs[:3]:
                    objects.append(
                        AffectedObject(
            ...
            if per_syscall:
                sorted_sys = sorted(
                    per_syscall.values(), key=lambda x: x.get("calls_per_sec", 0), reverse=True
                )
    ...
    def _extract_key_metrics(self, metrics: dict) -> dict:
        ...
        metric_paths = {
            AnomalyType.CPU_ANOMALY: [
                "cpu.global.cpu_usage_percent",
                "cpu.global.context_switches_per_sec",
            ...
            AnomalyType.MEMORY_ANOMALY: [
                "mem.system.available_percent",
                "mem.system.used_percent",
                "mem.events.direct_reclaim_per_sec",
                "mem.events.kswapd_wake_per_sec",
            ],
    ...
    def _load_default_rules(self):
                ...
                conditions=[
                    {
                        ...
                        "op": ">",
                        "value": self._th("cpu_usage_high", 90),
                ...
                conditions=[
                    {
                        "metric": "cpu.global.cpu_usage_percent",
                        "op": ">",
                        "value": self._th("cpu_usage_warn", 70),
        ...
        self.rules.append(
            Rule(
                ...
                conditions=[
                    {
                        "metric": "cpu.global.sched_avg_latency_ms",
                        "op": ">",
                        "value": self._th("sched_delay_high", 50),
        ...
        self.rules.append(
            Rule(
                ...
                conditions=[
                    {
                        "metric": "io.global.p99_latency_ms",
                        "op": ">",
                        "value": self._th("io_p99_high", 50),
        ...
        self.rules.append(
            Rule(
                ...
                conditions=[
                    {
                        "metric": "mem.system.available_percent",
                        "op": "<",
                        "value": self._th("mem_available_low", 10),
                ...
                conditions=[
                    {
                        "metric": "mem.system.available_percent",
                        "op": "<",
                        "value": self._th("mem_available_low", 10) / 2,
                        "threshold_key": "mem_available_low",
                        "threshold_factor": 0.5,
                        "description": "可用内存极低，有OOM风险",
                    },
                    {
                        "metric": "mem.events.oom_kill_total",
                        "op": ">",
                        "value": 0,
        ...
        self.rules.append(
            Rule(
                ...
                conditions=[
                    {
                        "metric": "lock.global.total_futex_wait_ms",
                        "op": ">",
                        "value": self._th("lock_wait_high", 10) * 10,
                        "threshold_key": "lock_wait_high",
